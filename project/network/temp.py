...
class SocialNetworkEnv(gym.Env):
    ...
    def reset_storedInfo(self):
        """
        Resets the storedInfo for all consumer nodes in the network.
        """
        for node, data in self.graph.nodes(data=True):
            
            if self.graph.nodes[node]["agentType"] == "consumer":
                self.graph.nodes[node]["storedInfo"] = []

    # visualizing the network
    def render(self, agent:  NewsAgent | FactCheckerAgent, mode="human"):
